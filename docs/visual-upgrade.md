...
All 33 Unity EditMode tests pass (2.86 seconds), along with 6 GIS tests and 1,089 coordinate checks. A cheap segment-bounds rejection reduced the suite from 14.07 seconds with unfiltered distance calculations; this is a desktop test timing, not a Quest performance measurement.
...
Ground cover no longer appears or disappears one 32 × 32 m chunk at a time. Candidate chunks are retained when their bounds intersect the view radius, then a reused buffer filters individual plant groups to 65 m for grain and 50 m for grass. The shader shrinks distant groups into their ground anchor over the final 35 percent of that radius. Both stereo eyes use the same viewer position. Mesh density is unchanged, and no transparent blending is added.

The native field study also renders the actual crop material onto a controlled quad at near, middle and beyond-range distances. Its readback checks pass: 4,096, 1,024 and zero visible pixels, with the same ground row (32) at full and half size. All four actual fields were inspected in `/tmp/ferraris-range-final/`; `ground-fade-*.png` are the isolated shader probe, not landscape screenshots. The range filter test covers a nearby group whose chunk centre lies outside the radius, distant groups within that same chunk, elevation independence, the exact radius and reuse after viewer movement.
