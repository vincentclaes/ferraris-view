...
Grain has varied heights and lean, two bent leaves per stem, tapered stems, slimmer ears with seed-row shading and height-weighted wind. Distant seed shading fades at subpixel size. Each instanced 16-stem clump has 1,088 vertices and 928 triangles (previously 736 triangles). Crops remain illustrative, not evidence of the exact local species or cultivar.

Plant jitter spans the full placement cell. Vegetation/livestock clearance follows full road segments with rounded ends, using a bounds rejection before exact distance calculations. Road vectors and widths are unchanged. Ground-cover rendering checks chunk bounds, then filters individual groups through a reused buffer: 65 m for grain and 50 m for grass. The shader shrinks groups into their ground anchor over the final 35 percent of this radius, replacing whole-chunk switching. Both stereo eyes use the same viewer position; no transparent blending or additional plant density is introduced.

Validation: 33 Unity EditMode tests and all 356 built desktop journey checks pass. The unchanged GIS/coordinate pipeline passed 6 tests and 1,089 checks. `bash scripts/smoke-desktop.sh -field-study` captures all four crop patches at walking height and close up, and tests the actual crop shader on an isolated quad. The near/middle/beyond-range readbacks contain 4,096/1,024/0 visible pixels, with identical ground row 32 at full/half size. Inspected evidence: `/tmp/ferraris-range-final/`; `ground-fade-*.png` are isolated probes, not landscape screenshots. The filter test covers nearby groups whose chunk centre is beyond range, distant groups within the same chunk, elevation independence, the exact radius and reuse after viewer movement.

The signed ARM64 Quest build from `4711476` passes offline-content, road-mask and all-19-voice gates. APK metadata excludes desktop field/shader/input studies. See the [current Quest receipt](quest-validation.md) and [browser preview](web.md). No headset is attached: sustained Quest timing, stereo, tracking and comfort remain unverified. The art remains stylised.
