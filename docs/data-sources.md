...
`tracing.json` records pixel-space manual corrections against the north-up 1024px preview and locks the crop origin/size. Roads follow visible historical road centres. The 57 building placements are approximate rectangles around red map symbols; the church is marked explicitly. Fields follow visible agricultural areas. Vegetation polygons represent orchards/enclosures; no dense forest was identified in this crop. `winksele_forest.geojson` retains the category name requested for the pipeline, with `landuse=orchard` to avoid mislabelling the content.
