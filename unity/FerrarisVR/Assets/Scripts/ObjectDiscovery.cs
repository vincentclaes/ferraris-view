...
namespace Ferraris
{
    public class ObjectDiscovery : MonoBehaviour
    {
        ...
        {
            ui.RemovePanel(panel);panel=null;ui.PanelOpen=false;ui.ClosePanel=null;
            if(stop){ui.ClearKeyboardFocus();Active=false;hint.SetActive(false);ring.enabled=false;Cursor.lockState=app.InWorld&&!app.IsXR?CursorLockMode.Locked:CursorLockMode.None;Cursor.visible=!app.InWorld||app.IsXR;}
        }
        void LateUpdate()
        {
